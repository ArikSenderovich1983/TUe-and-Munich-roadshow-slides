...
\title{Proactive Data-driven Scheduling of Business Processes}%: \\A Matter of Stochastic Durations and Resource Constraints}
% Proactive Scheduling of Business Processes with Stochastic Durations and Planned Resource Unavailability
\author{
    %Authors
...
apply process mining to infer unknown
...
Makespan of the stochastic problem. Our approach integrates data-driven Monte Carlo simulation with constraint programming to generate proactive schedules that guarantee, with
high probability, that 
the Makespan remains below a predefined threshold.
...
%\color{blue}{We found that ... here mention some results and the data we used for real-world on a hospital.} 

%can identify \btext{robust} scheduling solutions using synthetic data under varying levels of uncertainty, as well as %in a real-world business process where we effectively optimize the makespan.
%\btext{optimize the makespan of real-world business processes.}
%using either data, leveraging process mining approaches, or prior knowledge. 

%The conducted experiments demonstrate 


%All these characteristics pose a challenge to produce a schedule able to assign start times to activities corresponding to two or more projects that must be executed simultaneously while respecting the precedence between activities and limited resource
\end{abstract}
...
\input{sections/2_motivation_problem_def}
