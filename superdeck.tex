...
% Title
% ==========================================================
\title[AI for BPO Scheduling]{AI for Business Process Optimization:\\From Proactive Scheduling (from logs) to Decision-Focused Learning}
\author[Arik Senderovich]{Arik Senderovich}
\institute[York University]{York University \qquad \textbar \qquad Roadshow: TU/e \& TUM}
\date{January 2026}
...
\begin{frame}
  ...
  \begin{center}
    ...
    \color{darkgray}
    \ifshort
      TU/e version (20 min clean talk + Q/A)
    \else
      Munich version (45 min clean talk + Q/A)
    \fi
  \end{center}
\end{frame}
...
\begin{frame}{What is Business Process Optimization (BPO)?}
  \begin{columns}[T]
    \column{0.62\textwidth}
      \btext{BPO} = making \emph{optimal} decisions during process execution and redesign.
      \vspace{0.3cm}

      \begin{itemize}
        \item \btext{Decisions}: which case next, which resource, when to admit/schedule, where to route.
        \item \btext{Objectives}: time, cost, service levels, customer experience.
      ...
      \vspace{0.2cm}
      \small
      Today: one canonical BPO problem---\btext{case scheduling}.
    ...
    \column{0.38\textwidth}
      ...
      \vspace{0.1cm}
      \scriptsize
      Example process with multiple decision points (from BPO chapter).
  \end{columns}
\end{frame}

\begin{frame}{A simple taxonomy: where scheduling fits}
  \begin{columns}[T]
    \column{0.62\textwidth}
      \begin{block}{Operational decisions (during execution)}
        \begin{itemize}
          \item \btext{Task prioritization}: which waiting work item next?
          \item \btext{Resource allocation}: which resource executes it?
          \item \btext{Admission / case scheduling}: when to release or appoint a case?
          \item \btext{Path choice}: which route / option for this case?
        \end{itemize}
      \end{block}
      \vspace{0.1cm}
      \btext{Key message:} scheduling is the ``glue'' between BPM models and operational performance.

    \column{0.38\textwidth}
      \centering
      \includegraphics[width=\textwidth]{framework.pdf}
      \scriptsize
      BPO solution components: problem, data/uncertainty, model, method, evaluation.
  \end{columns}
\end{frame}
...
\begin{frame}{Core gap: calendars + uncertainty + process mining}
  \begin{itemize}
    \item Proactive scheduling handles \btext{uncertain durations} (classical OR / AI scheduling).
    \item BPM execution adds \btext{planned unavailability} and \btext{parameter estimation}.
  \end{itemize}

  \vspace{0.3cm}
  \begin{block}{Design principle}
    Build schedules with \btext{optimization} (CP) but \btext{validate under uncertainty} (simulation).
  \end{block}

  \vspace{0.3cm}
  \begin{center}
    \includegraphics[width=0.83\textwidth]{pipeline_rims3.png}
    \vspace{0.1cm}
    \scriptsize
...
% ==========================================================
\section{Lens 2: Smart Predict-and-Schedule (opening the black box)}
...
\begin{frame}{White-box idea: polynomial-time rules are sets of critical decisions}
  \begin{itemize}
    \item Many optimal/near-optimal dispatching rules are \btext{discrete} but \btext{structured}:
    \begin{itemize}
    ...
    \end{itemize}
    \item Convert these critical decisions into a smooth loss:
    \[
      L_{\mathrm{surr}} = \sum_{d \in \mathcal{K}} \phi_d(\theta^\star)\,
      \sigma\!\left(\frac{-m_d}{\lambda}\right)
    \]
    where \(\phi_d\) weights the \btext{cost} of getting decision \(d\) wrong.
  \end{itemize}
...
\begin{frame}{Results: synthetic robustness (why the surrogate matters)}
  \begin{center}
    \includegraphics[width=0.48\textwidth]{compare-methods-fixed-range.png}\hfill
    \includegraphics[width=0.48\textwidth]{improvement--percentage.png}
  \end{center}
  \vspace{-0.1cm}
  \begin{itemize}
    \item our surrogate stays stable even under heavy noise
    \item SPO+ can diverge; MSE/ranking plateau
  \end{itemize}
\end{frame}
  ...
  \begin{columns}[T]
    \column{0.58\textwidth}
      ...
      \begin{itemize}
        ...
        \item objective: reduce congestion (SCT proxy)
      \end{itemize}
      \vspace{0.2cm}
...
\begin{frame}{DayHospital results (headline slide)}
  \begin{columns}[T]
    \column{0.62\textwidth}
      \begin{block}{Key findings}
        \begin{itemize}
          ...
          \item \btext{$\sim$34\%} error reduction vs SPO+
          \item \btext{$\sim$10\%} improvement vs ranking baselines
        \end{itemize}
      \end{block}
      \vspace{0.1cm}
      \btext{Interpretation:} learning is guided by the scheduling rule’s structure and its real cost of mistakes.
    ...
    \column{0.38\textwidth}
      \centering
      % Placeholder for a clean summary graphic later:
      \fbox{\parbox{0.95\textwidth}{
        \vspace{0.15cm}
        \centering
        \textbf{Placeholder}\\
        Monthly bar chart of relative error\\
        \vspace{0.15cm}
      }}
      \vspace{0.1cm}
      \scriptsize
      Suggestion: generate a simple 12-month bar chart in ChatGPT UI.
  \end{columns}
\end{frame}
