...
\begin{frame}{What is Business Process Optimization (BPO)?}
  \btext{BPO} = making \emph{optimal} operational decisions during process execution (and designing policies for them).

  \vspace{0.3cm}
  \begin{itemize}
    \item<1-> \btext{Decisions}: \emph{what to do next} and \emph{with which resources}.
    \item<2-> \btext{Objectives}: time, cost, throughput, service levels, customer experience.
    \item<3-> \btext{Constraints}: capacity, calendars, deadlines, policies.
    \item<4-> \btext{Hard part}: uncertainty in arrivals, durations, and paths.
  \end{itemize}

  \vspace{0.35cm}
  \begin{block}{A simple taxonomy of BPO problem types (operational level)}
    \begin{itemize}
      \item<5-> \btext{Task prioritization} \& \btext{resource allocation} (dispatching)
      \item<6-> \btext{Case admission} \& \btext{case scheduling} \hfill \btext{(focus today)}
      \item<7-> \btext{Staffing} (dynamic capacity decisions)
      \item<8-> \btext{Path choice} (routing / treatment options)
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}{How do we solve BPO problems? (a reusable blueprint)}
  \begin{center}
    \includegraphics[width=0.93\textwidth]{framework.pdf}
  \end{center}
  \vspace{-0.1cm}
  \begin{columns}[T]
    \column{0.5\textwidth}
