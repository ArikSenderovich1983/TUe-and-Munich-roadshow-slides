...
% beamer already loads xcolor; avoid re-loading (option clash).
\usepackage{colortbl} % provides \cellcolor for tables
\usepackage{tikz}
...
\graphicspath{
  ...
  {valuation_driven_case_scheduling_IJCAI/}
}
...
\begin{frame}{Thesis: scheduling in business processes is fundamentally non-local}
  ...
  \begin{block}{What replaces them}
    ...
    robust proactive planning (BPSP) \(\rightarrow\) decision-aware learning (SASS) \(\rightarrow\)
    \iflong valuation \& incentives (capstone).\else valuation (third perspective).\fi
  \end{block}
  \vspace{0.2cm}
  \smallref{Framing background: Dumas et al., \emph{Fundamentals of Business Process Management}, 2nd ed., 2018.}
\end{frame}

% NEW: Addresses critique #1 — BPO taxonomy (book chapter)
% CORE
\begin{frame}{Where each contribution sits in the BPO landscape}
  \btext{BPO taxonomy (chapter):} problem families \(\times\) decision levels.
  ...
  \begin{tabular}{l|c|c|c|c}
    \toprule
    \textbf{Level} &
    \textbf{\shortstack{Task Prioriz. \&\\Resource Alloc.}} &
    \textbf{\shortstack{Case Admission\\\& Scheduling}} &
    \textbf{Staffing} &
    \textbf{Path Choice} \\
    \midrule
    \textbf{Strategic} &
    \cellcolor{black!3}Role / worklist design &
    \cellcolor{black!3}Admission \& schedule rule design &
    \cellcolor{black!3}Workforce / skill mix &
    \cellcolor{black!3}Process (re)design \\
    \midrule
    \textbf{Tactical} &
    \cellcolor{black!3}Task scheduling &
    \cellcolor{okgreen!15}\shortstack{\btext{Scheduling}\\{\scriptsize\color{darkgray}\btext{BPSP (IJCAI'25)}}} &
    \cellcolor{black!3}Shift scheduling &
    \cellcolor{black!3}Policy setting \\
    \midrule
    \textbf{Operational} &
    \cellcolor{okblue!15}\shortstack{\btext{Task allocation}\\{\scriptsize\color{darkgray}\btext{SASS (IJCAI'26)}}} &
    \cellcolor{black!3}Admission control &
    \cellcolor{black!3}Dynamic capacity control &
    \cellcolor{okorange!15}\shortstack{\btext{Case routing /}\\\btext{gateway control}\\{\scriptsize\color{darkgray}\btext{Valuation (IJCAI'26)}}} \\
    \bottomrule
  \end{tabular}

  \vspace{0.15cm}
  \smallref{Table adapted from the BPO chapter (Table ``Optimization problems at different levels'').}
\end{frame}
...
% CORE
\begin{frame}{BPSP is ``estimate then optimize''—but with distributions and calendars}
...
\iflong
% EXPAND
\begin{frame}{Proposition 1: buffered durations still lower-bound robust makespan with calendars}
  \begin{columns}[T]
    \column{0.52\textwidth}
      \begin{block}{Definition (PPE, brief)}
        Positive precedence expressions are built from primitives \(\texttt{before}(i,j)\)
        using only \(\wedge\) and \(\vee\).
      \end{block}

      \begin{block}{Classical result (Beck \& Wilson)}
        If feasibility constraints are a conjunction of PPEs, then choosing
        \[
          q_U=\frac{\Phi^{-1}(1-\alpha)}{\sqrt{U}}
        \]
        yields a deterministic makespan lower bound for the \((1-\alpha)\)-probabilistic makespan target.
      \end{block}

    \column{0.48\textwidth}
      \begin{block}{Calendar insight (BPSP)}
        Encode unavailability as \btext{dummy activities} / \btext{extended forbidden sets}:
        for each resource \(r\) and time \(t\) where \(v_{r,t}=0\), forbid overlaps.
        \vspace{0.1cm}

        Resource feasibility becomes a PPE-style conjunction:
        \[
          \phi \wedge \psi_u
        \]
        (precedence \(\phi\) + capacity+calendar \(\psi_u\)).
      \end{block}

      \begin{block}{Proposition 1 (BPSP)}
        Selecting \(q=q_U\) yields a lower-bound solution to the stochastic BPSP, even with calendars.
      \end{block}
  \end{columns}

  \smallref{Adapted from IJCAI'25 BPSP method section: PPE definition + Proposition ``Selecting \(q=q_U\) yields a lower-bound'' with calendars via dummy vacations/forbidden sets.}
\end{frame}
\fi
...
% NEW: Addresses critique #3 — why SPO+ fails (CORE)
\begin{frame}{Why SPO+ struggles in comparison-based scheduling rules}
  \begin{block}{The core difficulty}
    In scheduling, the decision map \(\hat{\theta}\mapsto \pi(\hat{\theta})\) is \btext{discontinuous}:
    a tiny change in predictions can flip a single comparison and yield a different schedule.
  \end{block}

  \begin{block}{Why this breaks generic decision-focused surrogates (intuition)}
    \begin{itemize}
      \item Regret is \btext{non-differentiable} because schedules jump discretely.
      \item SPO+ relies on a convex surrogate that behaves like a smooth approximation of the oracle.
      \item Comparison-based rules violate the smoothness intuition: one flipped critical decision can induce a large objective jump.
    \end{itemize}
  \end{block}

  \begin{block}{What SASS changes}
    SASS trains directly on the \btext{critical decisions} of the rule and weights errors by their local objective impact.
  \end{block}

  \smallref{See SASS draft: ``discrete and brittle dependence of optimal rules on predictions'' (abstract + introduction).}
\end{frame}
...
\begin{frame}{Instantiation (Problem 1): single-machine SCT with SPT}
  ...
  \]
  \smallref{See SASS paper: Problem 1 (single-machine SCT minimization via SPT).}
\end{frame}
...
\begin{frame}{Instantiation (Problem 2): flow shop makespan via Johnson’s rule}
  \btext{Setting:} two-machine flow shop \(F2\|C_{\max}\) with unknown processing times \((p_{i1},p_{i2})\).

  \vspace{0.15cm}
  \begin{block}{Johnson’s rule = two layers of critical decisions}
    Partition:
    \[
      G_1=\{i:p_{i1}<p_{i2}\},\quad G_2=\{i:p_{i1}\ge p_{i2}\}.
    \]
    Then sort \(G_1\) by increasing \(p_{i1}\), and \(G_2\) by decreasing \(p_{i2}\).
  \end{block}

  \begin{block}{SASS surrogate decomposes accordingly}
    Predict decision-relevant scores \((\hat d_i,\hat s_i)=f_\omega(x_i)\).
    \begin{itemize}
      \item \btext{Group loss} (assignment): \(L_{\mathrm{group}}=\sum_i |d_i|\,\sigma(-\hat d_i d_i/\lambda)\), where \(d_i=p_{i1}-p_{i2}\).
      \item \btext{Order loss} (within-group): weight each potentially wrong swap by \(\Delta_{i,j}=T_{j,i}-T_{i,j}\) (makespan impact).
    \end{itemize}
    Total: \(L_{\mathrm{Johnson}}=L_{\mathrm{group}}+L_{\mathrm{order}}\).
  \end{block}

  \smallref{Matches SASS draft: group vs ordering losses with weights \(|d_i|\) and \(\Delta_{i,j}\) (``Problem 2: Flow Shop Scheduling'').}
\end{frame}
\fi
...
\iflong
% EXPAND
\begin{frame}{Empirics: valuation-aware scheduling improves welfare over FIFO}
  \begin{center}
    \includegraphics[width=0.95\textwidth,height=0.78\textheight,keepaspectratio]{rq1_improvement_vs_fifo_results_seeds20.png}
  \end{center}
  \vspace{-0.15cm}
  \smallref{Valuation paper (IJCAI'26 draft): simulation comparison of FIFO vs valuation-driven scheduling (experiments section).}
\end{frame}
...
% NEW: Addresses critique #5 — synthesis is not a pipeline (P0)
% CORE
\begin{frame}{Synthesis: three attacks on the locality assumption}
  \begin{center}
    \begin{tikzpicture}[
      node distance=1.0cm and 1.2cm,
      box/.style={rounded corners, draw=black!45, thick, fill=black!2, inner sep=7pt, align=left},
      mid/.style={rounded corners, draw=darkred, very thick, fill=darkred!8, inner sep=8pt, align=center},
      arr/.style={-Latex, thick, draw=black!60}
    ]
      \node[box] (b) {\btext{BPSP}\\Non-locality across \btext{time} and \btext{scenarios}\\(calendars + uncertainty)};
      \node[box, below=of b] (s) {\btext{SASS}\\Non-locality in the \btext{learning objective}\\(critical decisions + swap costs)};
      \node[box, below=of s] (v) {\btext{Valuation}\\Non-locality across \btext{cases} and \btext{branches}\\(externalities + incentives)};

      \node[mid, right=3.0cm of s] (core) {\btext{Structure-aware BPO}\\Scheduling without locality};

      \draw[arr] (b.east) -- (core.west);
      \draw[arr] (s.east) -- (core.west);
      \draw[arr] (v.east) -- (core.west);
    \end{tikzpicture}
  \end{center}
  \vspace{0.15cm}
  \btext{Agenda:} connect logs, robust planning, learning, and incentives to control non-local scheduling in BPO.
\end{frame}
...
\begin{frame}{Key references (exact)}
  ...
  \begin{itemize}
    ...
    \item Smith. \emph{Various optimizers for single-stage production}. Naval Research Logistics Quarterly, 1956. % WSPT / Smith rule
    \item Pinedo. \emph{Scheduling: Theory, Algorithms, and Systems}. Springer, 5th ed., 2016.
